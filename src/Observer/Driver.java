...
public class Driver extends Employee implements Observer {
    ...
    public void receiveOffer(String nameCompany, VacancyType vacancyType, int salary) {
        if (this.salary <= salary && this.vacancyType == vacancyType) {
            System.out.printf("Водитель %s: Мне нужна эта работа! (компания: %s, %s; заработная плата: %d)\n",
                    name, nameCompany, vacancyType.toString(), salary);
            this.salary = salary;
        } else if (this.vacancyType != vacancyType) {
            System.out.printf("Водитель %s: Не мой профиль! (компания: %s, %s; заработная плата: %d)\n",
                    name, nameCompany, vacancyType.toString(), salary);

        } else {
            System.out.printf("Водитель %s: Я найду работу получше! (компания: %s, %s; заработная плата: %d)\n",
                    name, nameCompany, vacancyType.toString(), salary);
        }
